...
class WeekEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function create() {
		...
		var tracksSprite:FlxSprite = new FlxSprite(FlxG.width * 0.07, bgSprite.y + 435).loadGraphic(Paths.image('menus/story_mode/Menu_Tracks'));
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('menus/story_mode/menubackgrounds/menu_' + assetName)) || #end
			Assets.exists(Paths.getPath('images/menus/story_mode/menubackgrounds/menu_' + assetName + '.png', IMAGE), IMAGE)) {
				bgSprite.loadGraphic(Paths.image('menus/story_mode/menubackgrounds/menu_' + assetName));
				isMissing = false;
			}
		}
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('menus/story_mode/storymenu/' + assetName)) || #end
			Assets.exists(Paths.getPath('images/menus/story_mode/storymenu/' + assetName + '.png', IMAGE), IMAGE)) {
				weekThing.loadGraphic(Paths.image('menus/story_mode/storymenu/' + assetName));
				isMissing = false;
			}
		}
		...
		if(isMissing) {
			...
			missingFileText.text = 'MISSING FILE: images/menus/story_mode/storymenu/' + assetName + '.png';
		}
