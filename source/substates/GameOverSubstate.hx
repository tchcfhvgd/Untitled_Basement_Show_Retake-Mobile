...
class GameOverSubstate extends MusicBeatSubstate
{
	...
	public static var gameoBPM:Float = 67.5;
	var chromValue:Float;

	public static var fakeZoom:Float = 0.8;
	...
	override function create()
	{
		...
		FlxG.camera.focusOn(new FlxPoint(FlxG.camera.scroll.x + (FlxG.camera.width / 2), FlxG.camera.scroll.y + (FlxG.camera.height / 2) - 35));
		...
		Conductor.bpm = gameoBPM;
		FlxG.game.setFilters([]);
		FlxG.camera.setFilters([new ShaderFilter(chromatic)]);
	}
	...
	override function update(elapsed:Float)
	{
		...
		chromValue = FlxMath.lerp(chromValue, 0.5, 0.04);
		FlxG.camera.zoom = FlxMath.lerp(FlxG.camera.zoom, fakeZoom, 0.04);
	...
	function endBullshit():Void
	{
		if (!isEnding)
		{
			FlxTween.tween(FlxG.camera, {alpha: 0}, 2, {ease: FlxEase.sineInOut});
			FlxTween.num(fakeZoom, 4, 3, {ease: FlxEase.backInOut}, function(num) {fakeZoom = num;});
			isEnding = true;
