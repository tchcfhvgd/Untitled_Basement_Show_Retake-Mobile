...
class BaseOptionsMenu extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		FlxG.camera.scroll.x = -1000;
		FlxTween.tween(FlxG.camera.scroll, {x: 0}, 0.5, {ease: FlxEase.quadOut});
		
		if (!ClientPrefs.data.fromFlash) {
		...
		checker = new FlxBackdrop(Paths.image('menus/checker'), Y);
		...
		descText = new FlxText(500, 600, 780, "", 32);
		descText.setFormat(Paths.font("tbs.ttf"), 32, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		descText.scrollFactor.set();
		descText.borderSize = 2.4;
		add(descText);

		if (!ClientPrefs.data.fromFlash) {
		popTxt = new FlxText(0, 20, FlxG.width, OptionsState.popTxt.text, 20);
		...
		dTxt = new FlxText(0, 50, FlxG.width, OptionsState.dTxt.text, 20);
		...
		{
			var optionText:Alphabet = new Alphabet(155, 330, optionsArray[i].name, false);
			optionText.isMenuItem = true;
			optionText.setScale(0.7, 0.7);
